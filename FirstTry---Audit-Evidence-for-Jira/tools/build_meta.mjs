...
// ═══════════════════════════════════════════════════════════════════════════════
// UI LAYER 1: Generate TypeScript build metadata files for tests and runtime imports
// ═══════════════════════════════════════════════════════════════════════════════

// Generate ui_build_meta.ts
const uiBuildMetaTsPath = path.join(__dirname, '..', 'src', 'gadget-ui', 'src', 'ui_build_meta.ts');
const markerTime = meta.FT_BUILD_TIME_UTC.replace(/[-:]/g, '').replace(/\.\d{3}/, '');
const uiBuildMetaTsContent = `/**
 * ui_build_meta.ts - AUTO-GENERATED
 * 
 * Immutable UI build identifiers generated at build time.
 * DO NOT EDIT MANUALLY - regenerated by tools/build_meta.mjs
 */

export const UI_GIT_SHA = "${meta.UI_GIT_SHA}";
export const UI_GIT_TIME = "${meta.FT_BUILD_TIME_UTC}";
export const UI_BUILD_TIME_UTC = "${meta.FT_BUILD_TIME_UTC}";
export const UI_BUILD_MARKER = \`UI_MARKER_${markerTime}_\${UI_GIT_SHA.substring(0, 7)}\`;
`;
fs.writeFileSync(uiBuildMetaTsPath, uiBuildMetaTsContent, 'utf8');
console.log(`✅ Generated ${uiBuildMetaTsPath}`);

// Generate build/buildIdentity.gen.ts for UI layer
const uiBuildIdentityDir = path.join(__dirname, '..', 'src', 'gadget-ui', 'src', 'build');
if (!fs.existsSync(uiBuildIdentityDir)) {
  fs.mkdirSync(uiBuildIdentityDir, { recursive: true });
}
const uiBuildIdentityPath = path.join(uiBuildIdentityDir, 'buildIdentity.gen.ts');
const uiSha7 = meta.UI_GIT_SHA.substring(0, 7);
const uiBuildIdentityContent = `/**
 * buildIdentity.gen.ts - AUTO-GENERATED
 * 
 * UI build identity for dashboard and components.
 * DO NOT EDIT MANUALLY - regenerated by tools/build_meta.mjs
 */

export const UI_GIT_SHA = "${meta.UI_GIT_SHA}";
export const UI_GIT_SHA_SHORT = "${uiSha7}";
export const UI_BUILD_TIME_UTC = "${meta.FT_BUILD_TIME_UTC}";
export const UI_APP_VERSION = "${meta.FT_APP_VERSION}";
`;
fs.writeFileSync(uiBuildIdentityPath, uiBuildIdentityContent, 'utf8');
console.log(`✅ Generated ${uiBuildIdentityPath}`);
