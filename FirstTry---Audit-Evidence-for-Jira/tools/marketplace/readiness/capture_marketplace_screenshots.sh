...
# Layout A (dev/nested):   REPO_ROOT/atlassian/forge-app/
# Layout B (vendor/flat):  REPO_ROOT/
...
if [ "$CAPTURE_MODE" = "tunnel" ] && ! command -v forge >/dev/null 2>&1; then
  ...
  echo "  cd \"\$FORGE_APP_DIR\" && forge tunnel"
...
echo "[INFO] Installing npm dependencies in \$FORGE_APP_DIR..."
cd "$FORGE_APP_DIR"
