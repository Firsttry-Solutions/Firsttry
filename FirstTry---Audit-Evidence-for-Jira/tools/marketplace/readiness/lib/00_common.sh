...
# ── Forge app directory layout detection ─────────────────────────────────────
# Supports two layouts:
#   Layout A (dev/nested):   REPO_ROOT/atlassian/forge-app/  (Firsttry dev repo)
#   Layout B (vendor/flat):  REPO_ROOT/                       (flat Forge app at root)
# Override with FORGE_APP_DIR env var if needed.
if [ -z "${FORGE_APP_DIR:-}" ]; then
  if [ -d "$REPO_ROOT/atlassian/forge-app" ]; then
    FORGE_APP_DIR="$REPO_ROOT/atlassian/forge-app"
  else
    FORGE_APP_DIR="$REPO_ROOT"
  fi
fi
export FORGE_APP_DIR
