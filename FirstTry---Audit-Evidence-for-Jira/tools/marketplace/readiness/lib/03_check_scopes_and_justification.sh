...
if [ ! -d "$FORGE_APP_DIR/node_modules/yaml" ]; then
  ...
  if ! (cd "$FORGE_APP_DIR" && npm ci --prefer-offline --no-audit > "$PHASE_DIR/npm_ci_phase03.log" 2>&1); then
    phase_fail "npm ci failed (needed for yaml parser): see $PHASE_DIR/npm_ci_phase03.log" "$PHASE_DIR/npm_ci_phase03.log"
  fi
