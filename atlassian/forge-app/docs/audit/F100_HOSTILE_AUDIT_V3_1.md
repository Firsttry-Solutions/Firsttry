...
### Artifact-Level Contamination Scan (Phase 06) — 3-Layer Defense
Beyond code-level taint gates, Phase 06 performs **3-layer artifact contamination scanning** to detect UUID/correlationId leakage in the actual customer-delivered export artifact:

#### Layer A: Raw ZIP Bytes Scan
- Generates customer-delivered ZIP using `tests/determinism/exportZipArtifactGenerator.test.ts`
- Uses identical ZIP builder as `src/resolvers/ft_exportAccessPack_v1.ts` (buildZipArchive + makeDeterministicZip)
- Scans raw ZIP bytes with: `strings export_pack.zip | rg <pattern>`
- **Rationale**: Catches contamination in ZIP headers, metadata, compressed streams, or binary-encoded strings
- Evidence: ZIP size, SHA-256 hash, raw byte scan results

#### Layer B: Extracted ZIP Contents Scan
- Extracts ZIP with `unzip` (fail-closed: audit fails if unzip unavailable)
- Fixes file permissions (deterministic ZIP may set zero perms)
- Scans all extracted file contents with `rg -n <pattern>`
- **Rationale**: Validates actual file contents customers will see after extraction
- Evidence: List of extracted files, content scan results

#### Layer C: Pack File Directory Scan
- Scans individual pack files generated by `tests/determinism/exportArtifactGenerator.ts`
- **Rationale**: Validates pre-ZIP pack file generation (reviewPack.generatePackFiles)
- Evidence: List of pack files, content scan results

#### Contamination Patterns
All three layers fail if ANY of these patterns are found:
...
- If ANY layer finds contamination → Phase 06 **FAILS** immediately
- If unzip unavailable → Phase 06 **FAILS** (cannot verify Layer B)
- If ZIP generation fails → Phase 06 **FAILS**
- Evidence artifact: `PHASE_06_export_artifact_contamination_scan.txt` with all scan results

#### Why 3 Layers Are Stronger
- **Layer A (raw bytes)**: Catches contamination in ZIP internals (headers, compressed data, metadata)
- **Layer B (extracted)**: Verifies what customers actually see after unpacking
- **Layer C (pack files)**: Validates pre-ZIP generation (intermediate artifacts)
- **Combined**: No bypass possible — all representations of export data are scanned
- **Closes audit event loophole**: Since `src/audit` is excluded from code scans (legitimate event ID usage), artifact scans prove audit events cannot leak into customer exports
