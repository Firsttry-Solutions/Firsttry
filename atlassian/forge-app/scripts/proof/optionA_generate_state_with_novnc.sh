...
echo "   ✅ SAFE: Use local file directly (state.json is already saved)"
echo "   Local tests can use: tests/playwright/.auth/state.json"
echo "   (Do NOT load base64 into env vars for local runs)"
echo ""
...
echo "   To copy /tmp/state.b64 safely:"
echo "     - Open in editor: code /tmp/state.b64"
echo "     - Or copy from file explorer (never cat to terminal)"
echo "     - Paste into GitHub Secret FT_PLAYWRIGHT_STATE_B64"
echo ""
echo "⚠️  SECURITY REMINDERS (STRICT):"
echo "   - DO NOT run: cat /tmp/state.b64 in terminal"
echo "   - DO NOT export base64 into env vars locally"
echo "   - DO NOT paste base64 into logs or terminal output"
