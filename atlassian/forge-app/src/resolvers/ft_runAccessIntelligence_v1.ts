...
async function fetchAllUsersReadOnly(): Promise<any[]> {
  ...
  while (true) {
    try {
      const response = await api.asApp().requestJira(`/rest/api/3/users/search?startAt=${startAt}&maxResults=${maxResults}` as any, {
...
async function fetchAllProjectsReadOnly(): Promise<any[]> {
  ...
  while (true) {
    try {
      const response = await api.asApp().requestJira(`/rest/api/3/projects/search?startAt=${startAt}&maxResults=${maxResults}` as any, {
        headers: { 'Content-Type': 'application/json' },
      });
      const data: any = await response.json();
...
async function detectGlobalAdminsReadOnly(users: any[]): Promise<string[]> {
  ...
  for (const user of users) {
    try {
      ...
      const response = await api.asApp().requestJira(`/rest/api/3/user/groups?username=${encodeURIComponent(user.name || user.accountId)}` as any, {
        headers: { 'Content-Type': 'application/json' },
      });
