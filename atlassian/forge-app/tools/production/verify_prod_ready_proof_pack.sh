...
(
  ...
  while IFS= read -r filepath; do
    ...
    # run_prod_ready_audit.full.log has a post-seal annotation (PROOF_PACK_PACKHASH_FINAL)
    # appended after pass-2 sealing. Strip that line before hashing so the computed hash
    # matches the stored hash (which was computed before the annotation was appended).
    if [ "$filepath" = "09_release/run_prod_ready_audit.full.log" ]; then
      _STRIPPED="$(mktemp)"
      grep -v '^PROOF_PACK_PACKHASH_FINAL: ' "$filepath" > "$_STRIPPED" 2>/dev/null
      sha256sum "$_STRIPPED" 2>/dev/null | sed "s|${_STRIPPED}|${filepath}|" || {
...
# ==============================================================================
# Cross-check: FULL_LOG must contain BOTH packhash labels (two-hash model)
#
# PROOF_PACK_PACKHASH_PRESEAL = intermediate hash after pass-1 binding, before
#   PROOF_PACK summary lines are appended. Sealed in pass-2 binding.
# PROOF_PACK_PACKHASH_FINAL   = authoritative hash after pass-2 reseal. Written
#   AFTER sealing (not sealed itself), but MUST equal prod_ready_packhash.txt.
#
# The verifier enforces FINAL == prod_ready_packhash.txt (fail-closed).
# ==============================================================================
FULL_LOG_FILE="$PACK_DIR/09_release/run_prod_ready_audit.full.log"

# PRESEAL label
PRESEAL_LINE="$(grep -m1 '^PROOF_PACK_PACKHASH_PRESEAL: ' "$FULL_LOG_FILE" || true)"
if [ -z "$PRESEAL_LINE" ]; then
  echo "FAIL: full.log missing PROOF_PACK_PACKHASH_PRESEAL: label" >&2
  exit 1
fi
PRESEAL_VAL="${PRESEAL_LINE#PROOF_PACK_PACKHASH_PRESEAL: }"
if ! echo "$PRESEAL_VAL" | grep -qE '^[0-9a-f]{64}$'; then
  echo "FAIL: PROOF_PACK_PACKHASH_PRESEAL is not 64 lowercase hex: '$PRESEAL_VAL'" >&2
  exit 1
fi

# FINAL label
FINAL_LINE="$(grep -m1 '^PROOF_PACK_PACKHASH_FINAL: ' "$FULL_LOG_FILE" || true)"
if [ -z "$FINAL_LINE" ]; then
  echo "FAIL: full.log missing PROOF_PACK_PACKHASH_FINAL: label" >&2
  exit 1
fi
FINAL_VAL="${FINAL_LINE#PROOF_PACK_PACKHASH_FINAL: }"
if ! echo "$FINAL_VAL" | grep -qE '^[0-9a-f]{64}$'; then
  echo "FAIL: PROOF_PACK_PACKHASH_FINAL is not 64 lowercase hex: '$FINAL_VAL'" >&2
  exit 1
fi

# Enforce FINAL == prod_ready_packhash.txt (fail-closed)
if [ "$FINAL_VAL" != "$STORED_PACKHASH" ]; then
  echo "FAIL: PROOF_PACK_PACKHASH_FINAL in full.log does not equal prod_ready_packhash.txt" >&2
  echo "  full.log FINAL : $FINAL_VAL" >&2
  echo "  packhash file  : $STORED_PACKHASH" >&2
  exit 1
fi
