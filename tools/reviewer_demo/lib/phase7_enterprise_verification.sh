...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
  ...
  :
      ...
      failed_count=$(sha256sum -c "09_evidence_pack/evidence_manifest.sha256" 2>&1 | grep -c ": FAILED" || echo "0")
      [[ $failed_count -gt 0 ]] && failures+="Hash verification failed ($failed_count files). "
    fi
  else
    failures+="Missing evidence_manifest.sha256. "
  fi

  # ==== C) PROOF PACK ====
  if [[ -f "09_evidence_pack/PROOF_PACK_SHA256.txt" && -f "09_evidence_pack/evidence_manifest.sha256" ]]; then
    local expected=$(cat "09_evidence_pack/PROOF_PACK_SHA256.txt" 2>/dev/null | tr -d '\n\r ')
    local actual=$(sha256sum "09_evidence_pack/evidence_manifest.sha256" 2>/dev/null | cut -d' ' -f1)
    if [[ "$expected" != "$actual" ]]; then
      failures+="Proof pack hash mismatch. "
    fi
  else
    failures+="Missing proof pack hash files. "
  fi

  # ==== D) NO PLACEHOLDERS ====
  log_info "Checking for placeholder content..."
  local placeholder_count=0
  for pattern in "TODO" "mock" "placeholder" "example"; do
    local found=$(find "08_governance_snapshot/derived" -name "*.json" -exec grep -l "$pattern" {} \; 2>/dev/null | wc -l || echo "0")
    [[ $found -gt 0 ]] && ((placeholder_count+=1)) && failures+="Found placeholder pattern '$pattern'. "
  done

  # ==== FINAL VERDICT ====
  local verdict="PASS"
  [[ -n "$failures" ]] && verdict="FAIL"

  # Write artifacts
  echo "$verdict" > "FINAL_REVIEWER_VERDICT.txt"

  # Build failures array
  local failures_json="[]"
  if [[ -n "$failures" ]]; then
    # Convert space-separated failures to JSON array
    local -a failure_items=()
    for item in $failures; do
      failure_items+=("$item")
    done
    failures_json=$(printf '%s\n' "${failure_items[@]}" | jq -Rs 'split("\n") | map(select(length > 0))')
  fi

  # Build stats object
  local stats_json=$(jq -n \
    --arg raw_req "$raw_req_count" \
    --arg hash_verified "$verified_count" \
    --arg hash_failed "$failed_count" \
    --arg placeholder "$placeholder_count" \
    '{raw_request_count: ($raw_req | tonumber), hash_verified: ($hash_verified | tonumber), hash_failed: ($hash_failed | tonumber), placeholder_count: ($placeholder | tonumber)}')

  local pass_bool=$([ "$verdict" = "PASS" ] && echo "true" || echo "false")
  local timestamp=$(date -u +'%Y-%m-%dT%H:%M:%SZ')

  local report=$(jq -n \
    --argjson pass "$(echo $pass_bool)" \
    --argjson failures "$failures_json" \
    --argjson stats "$stats_json" \
    --arg verified_at "$timestamp" \
    '{pass: $pass, failures: $failures, stats: $stats, verified_at_utc: $verified_at}')

  echo "$report" > "VERIFIER_REPORT.json"

  # Output verdict
  if [[ "$verdict" == "PASS" ]]; then
    log_info "✓ Enterprise verification PASSED"
    echo ""
    echo "======================================"
    ...
    log_error "✗ Enterprise verification FAILED"
    echo ""
    echo "======================================"
...
# Exit if called directly (for testing)
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  if [[ $# -eq 2 ]]; then
    run_enterprise_verification "$1" "$2"
  else
    echo "Usage: $0 <evidence_root> <mode:LIVE|OFFLINE>"
    exit 1
  fi
fi
